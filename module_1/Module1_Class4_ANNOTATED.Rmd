...
# Create a combined "major smoking-caused disease" variable
# This is not required but I thought it would be interesting to define Disease = LC OR CHD
nmes_data <- nmes_data %>%
  mutate(
...
nmes_data <- nmes_data %>%
  mutate(disease = factor(lc5 == "LC" | chd5 == "CHD", 
                          levels=c(FALSE, TRUE), 
                          labels=c("No disease", "Disease (MSCD)")))
...
It seems that after adjusting for the confounding variables, age and sex, ever smokers have `r abs(round(100*(1 - (model3_tab %>% filter(term == "eversmkEver smoker") %>% pull(estimate))), 0))` percent higher odds of disease compared to otherwise similar never smokers.
...
nmes_data <- nmes_data %>%
  mutate(
    disease = factor(
      #let's not forget the parentheses next time addy D:
      (lc5 == 1) | (chd5 == 1),
      levels = c(FALSE,TRUE),
      labels = c(0,1)
    ),
...
nmes_data$beltuse  <- as.factor(nmes_data$beltuse)
nmes_data$marital  <- as.factor(nmes_data$marital)
nmes_data$educate  <- as.factor(nmes_data$educate)
...
```{r resultslabelplot1, echo = FALSE}
nmes_data <- read_csv("module_1/nmesUNPROC.csv")
